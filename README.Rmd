...
Special tools are provided enabling testing of R packages that make API calls. 

## What's Inside The Tin

The following functions/methods are provided:

- `call_mitm`:	Spawn a mitmdump background process with custom args and return the process id
- `check_check`:	Check the HAR from the mitm_check() call
- `install_mitm`:	Helper to get mitmproxy installed
- `install_mitm_certificate`:	Helper to install mitmproxy root CA certificate
- `mitm_check`:	R CMD check a package (source tree), capturing network calls.
- `mitm_help`:	Show the mitmdump help screen
- `mitm_status`:	Check on the status of an mitmproxy process created with 'mitm_start()'
- `start_mitm`:	Spawn a mitmdump background process in "HAR capture" mode and return the process id
- `stop_mitm`:	Stop the mitmproxy background process and retrieve the generated HAR file
